...
const ClientDashboard = () => {
  ...
  const requestJson = async (url: string, token: string, init: RequestInit = {}) => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      setLoading({ profile: false, lawyers: false, cases: false, documents: false, messages: false, sessions: false });
      return () => controller.abort();
    }

    const load = async () => {
      setError("");
      setSectionErrors({ profile: "", lawyers: "", cases: "", documents: "", messages: "", sessions: "" });
      setLoading({ profile: true, lawyers: true, cases: true, documents: true, messages: true, sessions: true });

      const run = async (key: keyof typeof loading, fn: () => Promise<void>) => {
        try {
          await fn();
        } catch (err: any) {
          const message = err?.message || t(`client_dashboard.failed_load_${key}` as any);
          setSectionErrors((prev) => ({ ...prev, [key]: message }));
          if (key === 'profile') setError(message);
        } finally {
      ...
      await Promise.all([
        run('lawyers', async () => {
          try {
            const data = await requestJson('/api/cases/me/lawyers', token, { signal: controller.signal });
            ...
            throw new Error('Unexpected lawyers response shape');
          } catch {
            const fallback = await LawyersAPI.list(token);
            setLawyers((fallback as any).lawyers || fallback || []);
          }
        }),
        run('cases', async () => {
        ...
        run('documents', async () => {
          const data = await requestJson('/api/documents', token, { signal: controller.signal });
          setDocuments(Array.isArray(data) ? data : (data?.documents || []));
        }),
        run('messages', async () => {
          const data = await requestJson('/api/communication', token, { signal: controller.signal });
          setMessages(data?.messages || []);
        }),
        run('profile', async () => {
          const data = await requestJson('/api/auth/me', token, { signal: controller.signal });
          setProfile(data?.user || null);
        }),
        run('sessions', async () => {
          const data = await requestJson('/api/courtsessions', token, { signal: controller.signal });
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          typeof payload === 'string'
            ? payload
            : payload?.error || payload?.message || t('client_dashboard.upload_failed')
        );
      }
      if (payload) {
      ...
      setError(err?.message || t('client_dashboard.upload_failed'));
    }
  };
  ...
  <main id="main-dashboard-section" className="container mx-auto px-4 py-6">
      <h1 className="text-xl sm:text-2xl md:text-3xl font-bold mb-4 text-primary">{t('client_dashboard.title')}</h1>
    ...
    <div className="flex flex-col gap-4">
          ...
          <section className="bg-gradient-to-br from-blue-50 to-white rounded-2xl shadow-lg p-4 sm:p-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-base sm:text-lg md:text-xl font-semibold">{t('client_dashboard.profile')}</h2>
            </div>
            {loading.profile ? (
              <div className="flex items-center gap-2 text-gray-400 animate-pulse">Loading profile…</div>
            ) : sectionErrors.profile ? (
              <div className="text-sm text-red-600">{sectionErrors.profile}</div>
            ) : profile ? (
              <div className="grid grid-cols-1 gap-2 text-gray-700">
                <div><span className="font-semibold">{t('client_dashboard.name')}:</span> {profile.full_name || profile.fullName}</div>
                <div><span className="font-semibold">{t('client_dashboard.email')}:</span> {profile.email}</div>
              </div>
            ) : (
              <div className="flex items-center gap-2 text-gray-400 animate-pulse"><svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><path d="M8 12h.01M16 12h.01M12 16h.01" /></svg> {t('client_dashboard.loading_profile')}</div>
            )}
          </section>
          ...
          <section className="bg-gradient-to-br from-green-50 to-white rounded-2xl shadow-lg p-4 sm:p-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-base sm:text-lg md:text-xl font-semibold">{t('client_dashboard.real_time_chat')}</h2>
            </div>
            {/* TODO: Add recipient selection UI */}
            {user && user.id ? (
              <Chat user={user} recipient={recipient || { id: "demo-lawyer", full_name: "Demo Lawyer" }} />
            ) : (
              <div className="text-gray-400">{t('client_dashboard.loading_chat')}</div>
            )}
          </section>
          {/* My Lawyers */}
          <section className="bg-gradient-to-br from-teal-50 to-white rounded-2xl shadow-lg p-4 sm:p-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-base sm:text-lg md:text-xl font-semibold">{t('client_dashboard.my_lawyers')}</h2>
            </div>
            <ul className="divide-y divide-teal-100">
              {loading.lawyers ? (
                <li className="py-2 text-gray-400 animate-pulse">Loading lawyers…</li>
              ) : sectionErrors.lawyers ? (
                <li className="py-2 text-red-600 text-sm">{sectionErrors.lawyers}</li>
                ...
                connectedLawyers.map((l: any) => {
                  ...
                  return (
                    <li key={lid || l.email || l.full_name} className="py-2 flex items-center justify-between">
                      ...
                      <div className="text-xs text-gray-500 whitespace-nowrap">{count} {count === 1 ? t('client_dashboard.case_singular') : t('client_dashboard.case_plural')}</div>
                    </li>
                  );
                })
              ) : (
                <li className="text-gray-400">{t('client_dashboard.no_connected_lawyers')}</li>
              )}
            </ul>
          </section>
  {/* My Cases (Row 4) */}
  <section className="bg-gradient-to-br from-purple-50 to-white rounded-2xl shadow-lg p-4 sm:p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-base sm:text-lg md:text-xl font-semibold">{t('client_dashboard.my_cases')}</h2>
          </div>
          <div className="overflow-x-hidden">
            <table className="w-full table-fixed text-left rounded-lg">
              <thead className="bg-purple-100">
                <tr>
                  <th className="py-2 px-2 sm:px-3 font-semibold w-[40%] break-words">{t('client_dashboard.title_label')}</th>
                  <th className="py-2 px-2 sm:px-3 font-semibold w-[30%]">{t('client_dashboard.status_label')}</th>
                  <th className="py-2 px-2 sm:px-3 font-semibold hidden md:table-cell">{t('client_dashboard.status_steps')}</th>
                  <th className="py-2 px-2 sm:px-3 font-semibold hidden md:table-cell">{t('client_dashboard.progress_label')}</th>
                  <th className="py-2 px-2 sm:px-3 font-semibold hidden md:table-cell">{t('client_dashboard.details_label')}</th>
                </tr>
              </thead>
              <tbody>
                {loading.cases ? (
                  <tr>
                    <td colSpan={5} className="text-center text-gray-400 py-4">Loading cases…</td>
                  </tr>
                ) : sectionErrors.cases ? (
                  <tr>
                    <td colSpan={5} className="text-center text-red-600 py-4 text-sm">{sectionErrors.cases}</td>
                  </tr>
                ) : cases.length > 0 ? (
                  cases.map((c) => {
                    const statusList = [
                      { label: t('client_dashboard.open'), icon: "🟢", color: "text-green-600", tooltip: t('client_dashboard.open_tip') },
                      { label: t('client_dashboard.in_progress'), icon: "🟡", color: "text-yellow-600", tooltip: t('client_dashboard.in_progress_tip') },
                      { label: t('client_dashboard.awaiting_judgment'), icon: "🔵", color: "text-blue-600", tooltip: t('client_dashboard.awaiting_judgment_tip') },
                      { label: t('client_dashboard.closed'), icon: "⚫", color: "text-gray-600", tooltip: t('client_dashboard.closed_tip') }
                    ];
                    ...
                    return (
                      <tr key={c.id || c._id} className="border-b last:border-b-0 hover:bg-purple-100 transition text-sm sm:text-base">
                        ...
                        <td className="py-2 px-2 sm:px-3 text-xs text-gray-700 hidden md:table-cell">
                          <div><span className="font-semibold">{t('client_dashboard.filed')}:</span> {c.filedDate ? new Date(c.filedDate).toLocaleDateString() : t('client_dashboard.na')}</div>
                          <div><span className="font-semibold">{t('client_dashboard.lawyer')}:</span> {c.lawyerName || t('client_dashboard.na')}</div>
                          <div><span className="font-semibold">{t('client_dashboard.type')}:</span> {c.type || t('client_dashboard.na')}</div>
                        </td>
                      </tr>
                    );
                  })
                  ...
                  <tr>
                    <td colSpan={5} className="text-center text-gray-400 py-4">{t('client_dashboard.no_cases_found')}</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <section className="bg-gradient-to-br from-blue-50 to-white rounded-2xl shadow-lg p-4 sm:p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-base sm:text-lg md:text-xl font-semibold">{t('client_dashboard.virtual_court_sessions')}</h2>
          </div>
          <ul className="divide-y divide-blue-100">
            {loading.sessions ? (
              <li className="text-gray-400 animate-pulse py-2">Loading court sessions…</li>
            ) : sectionErrors.sessions ? (
              <li className="py-2 text-red-600 text-sm">{sectionErrors.sessions}</li>
            ) : sessions.filter(s => {
              ...
              sessions.filter(s => {
                ...
                <li key={s.id || s._id} className="py-2 flex justify-between items-center hover:bg-blue-100 rounded transition cursor-pointer" tabIndex={0}>
                  <span>
                    <span className="font-semibold text-blue-800">{s.case?.title || t('client_dashboard.court_session')}</span>
                  ...
                  {s.is_virtual && (
                    <button
                      ...
                      tabIndex={0}
                      aria-label={t('client_dashboard.join_virtual_session')}
                    >
                      {t('client_dashboard.join_virtual_session')}
                    </button>
                  )}
                </li>
              ))
            ) : (
              <li className="text-gray-400">{t('client_dashboard.no_virtual_sessions')}</li>
            )}
          </ul>
        </section>
  {/* Documents (Row 5) */}
  <section className="bg-gradient-to-br from-pink-50 to-white rounded-2xl shadow-lg p-3 sm:p-5 overflow-x-hidden text-[13px] sm:text-sm">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 mb-3">
            ...
            <h2 className="text-sm sm:text-lg md:text-xl font-semibold">{t('client_dashboard.documents')}</h2>
            <select
            ...
            >
              <option value="">{t('client_dashboard.select_case')}</option>
              {cases.map((c) => (
            ...
            <button
              ...
              disabled={!selectedCaseId}
              tabIndex={0}
              aria-label={t('client_dashboard.upload_document')}
            >
              + {t('client_dashboard.upload_document')}
            </button>
          </div>
          <ul className="divide-y divide-pink-100 break-words">
            {loading.documents ? (
              <li className="text-gray-400 animate-pulse py-2">Loading documents…</li>
            ) : sectionErrors.documents ? (
              <li className="py-2 text-red-600 text-sm">{sectionErrors.documents}</li>
            ) : documents.length > 0 ? (
              documents.map((d) => (
                <li key={d.id || d._id} className="py-2 flex justify-between items-center gap-2 hover:bg-pink-100 rounded transition cursor-pointer" tabIndex={0}>
                  ...
                  <a
                    ...
                    tabIndex={0}
                  >
                      {t('client_dashboard.view_download')}
                  </a>
                </li>
              ))
            ) : (
                <li className="text-gray-400">{t('client_dashboard.no_documents_found')}</li>
            )}
          </ul>
        </section>
  ...
  <section className="bg-gradient-to-br from-green-50 to-white rounded-2xl shadow-lg p-4 sm:p-5">
          <div className="flex items-center gap-2 mb-2">
            <svg className="w-7 h-7 text-green-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M21 12c0 4.418-4.03 8-9 8a9.77 9.77 0 01-4-.8l-4 1 1.1-3.3A7.96 7.96 0 013 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
            <h2 className="text-base sm:text-lg md:text-xl font-semibold">{t('client_dashboard.search_lawyers')}</h2>
          </div>
            <ul className="divide-y divide-green-100">
            {loading.lawyers ? (
              <li className="text-gray-400 animate-pulse py-2">Loading lawyers…</li>
            ) : sectionErrors.lawyers ? (
              <li className="py-2 text-red-600 text-sm">{sectionErrors.lawyers}</li>
            ) : lawyers.length > 0 ? (
              lawyers.map((l) => (
                <li key={l.id || l._id} className="py-2 flex items-center justify-between hover:bg-green-100 rounded transition" tabIndex={0}>
                  ...
                  <div className="flex items-center gap-2">
                    {connectedLawyerIds.has((l._id || l.id || '').toString()) ? (
                      <span className="px-2 py-1 bg-gray-200 text-gray-700 rounded text-xs">{t('client_dashboard.connected')}</span>
                    ) : (
                      isClient ? (
            <button
              ...
              setConnectOpen(true);
                        ...
                        >
                          {t('client_dashboard.connect')}
                        </button>
                      ) : (
                        <span className="text-xs text-gray-400">{t('client_dashboard.login_as_client_connect')}</span>
                      )
                    )}
                  </div>
                </li>
              ...
              <li className="text-gray-400">{t('client_dashboard.no_lawyers_found')}</li>
            )}
            </ul>
        </section>
        </div>
    ...
    <Dialog open={connectOpen} onOpenChange={setConnectOpen}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('client_dashboard.request_connect')}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium mb-1">{t('client_dashboard.case_title')} <span className="text-red-600">*</span></label>
            <Input value={connectTitle} onChange={(e) => setConnectTitle(e.target.value)} placeholder={t('client_dashboard.case_title_placeholder')} />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">{t('client_dashboard.case_description')} <span className="text-red-600">*</span></label>
            <Textarea value={connectDescription} onChange={(e) => setConnectDescription(e.target.value)} placeholder={t('client_dashboard.case_description_placeholder')} className="min-h-[100px]" />
            <p className="text-xs text-gray-500 mt-1">{t('client_dashboard.minimum_chars')}</p>
          </div>
        </div>
        <DialogFooter>
          <button
            ...
            onClick={async () => {
              ...
              if (title.length < 3 || desc.length < 10) {
                setError(t('client_dashboard.validation_error'));
                return;
              }
              try {
                setConnecting(true);
                ...
                setError(t('client_dashboard.failed_connect'));
              } finally {
                setConnecting(false);
              }
            }}
            disabled={connecting}
          >
            {connecting ? t('client_dashboard.sending') : t('client_dashboard.send_request')}
          </button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
